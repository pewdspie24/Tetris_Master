...
pygame.display.set_caption("tEtRiS_cUk_mAnH")
...
def redraw_screen():
    ...
    global timer_text
    timer_text = my_font.render(str(round((pygame.time.get_ticks()-x) / 1000, 2)), True, WHITE)
...
while menu:
    ...
    title=text_format("tEtRiS_cUk_mAnH", font, 90, yellow)
    if selected=="start":
    ...
    screen.blit(text_start, (screen_width // 2 - (start_rect[2] // 2)+3, 360))
    screen.blit(text_highscore, (screen_width // 2 - (highscore_rect[2] // 2)+3, 420))
    screen.blit(text_quit, (screen_width // 2 - (quit_rect[2] // 2)+3, 480))
    pygame.display.update()
    clock.tick(FPS)
    pygame.display.set_caption("tEtRiS_cUk_mAnH")
